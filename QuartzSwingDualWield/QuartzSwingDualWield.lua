--[[
	Modified Copy of original Swing
...
local function OnUpdate()
	if swingmode ~= 0 then return swingbar:Hide() end
...
function Swing:OnInitialize()
	...
	Quartz3:RegisterModuleOptions(MODNAME, getOptions, L["Swing"].." - Dualwield")
...
function Swing:OnEnable()
	...
	if not swingbar then
		swingbar = CreateFrame("Frame", "Quartz3SwingBarDualWield", UIParent, "BackdropTemplate")
...
function Swing:PLAYER_ENTER_COMBAT()
	...
	if math_abs(offhandlow - offhandhigh) > 0.1 and playerclass ~= "DRUID" then
		swingmode = 0
	end
end
...
do
	...
	function getOptions()
		options = options or {
		type = "group",
		name = L["Swing"].." - Dualwield",
		desc = L["Swing"].." - Dualwield",
		get = getOpt,
		set = setOpt,
		order = 600,
		args = {
